document.addEventListener("DOMContentLoaded", function() {
    ...
    // Sample questions and answers with tags and image responses
    var qaData = [
{ question: "লজিকের প্রোগ্রামসমূহ", tags: ["Logic programme","কি কি পড়ানো হয়"] , answer: " ✪ HSC Academic ✪ HSC Preparation   ✪ বিশ্ববিদ্যালয় ভর্তি প্রোগ্রাম  (বিজ্ঞান,মানবিক,ব্যবসায় শিক্ষা,বিভাগ পরিবর্তন) ✪ মেডিলজিক মেডিকেল ভর্তি প্রোগ্রাম (মেডিকেল+ভাসির্টি Combined Special Batch)  ✪ নার্সিং ভর্তি প্রোগ্রাম  (বিএসসি ইন নার্সিং , ডিপ্লোমা ইন নার্সিং , ডিপ্লোমা ইন মিডওয়াইফারি )" },
        { question: "লজিকের পরিচালক", tags: ["director","rafiqul islam","rofikul","rafikul","পরিচালক"] , answer: "MD. Rafiqul Islam ◑ B.Sc in CSE (HSTU - 1st Class 1st) ◑ M.Sc in CSE (RUET) ◑ Experience : 12 years +  ◑ Writer : Logic Math ◑ Subject : Math ", image: "Rafikul islam.jpg" },
    ...
    //    { question: "Show me a cat", tags: ["cat", "picture"], answer: "Here's a cute cat for you!" },
    ...
    // Send the initial bot message when the page loads
    sendInitialBotMessage();

    function sendInitialBotMessage() {
        var initialMessage = "How can I help you?";
        simulateBotTyping({ text: initialMessage });
        displaySuggestions(generateSuggestions()); // Display suggestions immediately after the initial message
    }
    ...
    function getResponse(message) {
        ...
        for (var i = 0; i < qaData.length; i++) {
            ...
            if (question.includes(lowercaseMessage) || tags.some(tag => lowercaseMessage.includes(tag))) {
                return qaData[i].image ? { text: qaData[i].answer, image: qaData[i].image } : { text: qaData[i].answer };
            }
        }
    ...
    function simulateTyping(element, response) {
        var index = 0;
        var typingInterval = setInterval(function() {
            if (index < response.text.length) {
                element.innerText += response.text.charAt(index);
                index++;
            } else {
